    ...
    <div class="track-album-change" v-else>
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="上传配图：" class="lable_zhu">
          <!-- <el-upload :multiple='true' :action="this.action" list-type="picture-card" :on-change='changeBtn' :file-list="ruleForm.picList" :on-success="uploadSuccess" :before-upload="beforeUpload">
          ...
          <!-- newPic -->
          <el-upload :multiple='true' action="#" list-type="picture-card" :on-change='changeBtn' :limit="100" :on-remove='handleRemove' :on-success="uploadSuccess" :file-list="ruleForm.picList" :before-upload="beforeUpload" :auto-upload="false" :http-request="uploadFile" ref="upload">
            <i class="el-icon-plus"></i>
          </el-upload>
          <el-progress v-show="progressFlag" :percentage="progressPercent" style="width:270px;"></el-progress>
          <el-button v-show="progressBtn" @click="subPicForm">提交上传</el-button>
...
export default {
  ...
  methods: {
    changeBtn(file, fileList) {
      // if (this.newImg.length < 10) {
      //   this.newImg.push(file)
      // }

      //  fileList = [...fei, ...this.newImg]

      // 确保是最后一次
      console.log(fileList.length)
      if (fileList.length > 10 && file.status == 'ready') {
        let total =
      ...
      util.ajax
        ...
        .then(res => {
          ...
          res.data.data.forEach((item, index) => {
            ...
            temp.url = this.$store.getters.golbleImage + target
            this.ruleForm.picList.push(temp)
          })
          ...
          //     'small_' +
          //     item.url.substring(site + 1, item.url.length)
          //   template[index].url = this.$store.getters.golbleImage + target
